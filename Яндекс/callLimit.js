  ...
  const wrap = (...args) => {
    ...
    if (count < limit) {
      ...
      if (count === limit && typeof callback === "function") {
        // ✅ вызываем ТОЛЬКО если callback передан и он — функция
...
Массив ...args нужен, чтобы работать с любым количеством аргументов

Если бы мы не использовали ...args, нам пришлось бы жёстко задавать параметры функции, 
и мы бы потеряли гибкость.

Цикла в данной задаче нет, только многократный вызов функции и концепция замыкания. 
*/
